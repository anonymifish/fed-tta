...
def finetune_test_cifar(configs):
...
def finetune_test_domain(configs):
...
def fedavg_finetune():
    ...
    file_handler = logging.FileHandler(os.path.join(save_path, 'fedavg_finetune.log'))
    ...
    torch.save(
        ...
        os.path.join(configs.checkpoint_path, configs.finetune_model_name)
    )

    setattr(configs, 'step', 'test')
    if configs.dataset in ['cifar10', 'cifar100']:
        finetune_test_cifar(configs)
    elif configs.dataset in ['digit-5', 'PACS', 'office-10', 'domain-net']:
        finetune_test_domain(configs)
