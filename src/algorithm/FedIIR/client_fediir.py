...
class FedIIRClient(BaseClient):
    def __init__(self, cid, device, backbone, configs):
        super(FedIIRClient, self).__init__(cid, device, backbone, configs)

        self.penalty_weight = configs.penalty_weight
    ...
    def train(self, grad_mean):
        ...
        for epoch in range(self.epochs):
            for data, target in self.train_dataloader:
                ...
                grad_client = autograd.grad(loss, self.backbone.fc.parameters(), create_graph=True)
                penalty_value = 0
                for g_client, g_mean in zip(grad_client, grad_mean):
                    penalty_value += (g_client - g_mean).pow(2).sum()
                loss = loss + self.penalty_weight * penalty_value
                loss.backward()
                self.optimizer.step()
